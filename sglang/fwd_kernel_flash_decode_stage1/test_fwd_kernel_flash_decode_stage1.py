...
q = torch.randn(batch_size, head_num, seq_len, d_model, device="npu", dtype=torch.float16)
k = torch.randn(batch_size, head_num, seq_len, d_model, device="npu", dtype=torch.float16)
v = torch.randn(batch_size, head_num, seq_len, d_model, device="npu", dtype=torch.float16)
Req_to_tokens = torch.randint(0, seq_len, (batch_size, seq_len), device="npu", dtype=torch.int32)
B_req_idx = torch.arange(batch_size, device="npu", dtype=torch.int32)
B_Seqlen = torch.full((batch_size,), seq_len, device="npu", dtype=torch.int32)

# 中间输出张量
mid_out = torch.zeros(batch_size, head_num, seq_len // block_seq, d_model, device="npu", dtype=torch.float16)
mid_out_logsumexp = torch.zeros(batch_size, head_num, seq_len // block_seq, device="npu", dtype=torch.float16)
